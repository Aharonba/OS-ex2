...
void start_udp_client(const char *hostname, int port, int *fd)
{
    ...
    *fd = socket(AF_INET, SOCK_DGRAM, 0);
    ...
    int n = sendto(*fd, message, strlen(message), 0, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
    if (n < 0)
    {
        perror("sendto");
        close(*fd);
...
int main(int argc, char *argv[])
{
    ...
    if (execute_program)
    {
        ...
        else if (pid == 0)
        {
            ...
            if (output_fd != -1)
            {
                std::cout << "im here 80" << std::endl;

                dup2(output_fd, STDOUT_FILENO);
                std::cout << "im here 82" << std::endl;
                close(output_fd);
            }
