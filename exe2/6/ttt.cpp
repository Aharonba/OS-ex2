...
int main(int argc, char *argv[])
{
    ...
    while (!gameOver(board, &winner))
    {
        ...
        else // User's turn
        {
            ...
            {
                printf("You tried to select 3 times in a row in an occupied place or you entered an incorrect character or the connection was disconnected\n");
                exit(1);
            }
        }
    }
...
bool putBoardPosition(int number, int arr[][3], int player)
{
    ...
    else
    {
        ...
        std::cout << "Error: Position already taken\n"
                  << std::endl;
